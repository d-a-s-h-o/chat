...
include('modules/b65k/base65536.php');
use \Hevertonfreitas\Base65536\Base65536;

include('modules/tictactoe/tictactoe.php');
...
function route(){
	...
	if(!isset($_REQUEST['action'])){
		...
		$arg='';
		send_links();
	}elseif($_REQUEST['action']==='games'){
		check_session();
		if(isset($_REQUEST['game']) && isset($_REQUEST['match'])){
			send_games($_REQUEST['game'],$_REQUEST['match'],$_REQUEST['partner'],$_REQUEST['choice'],$_REQUEST['move']);
		}else{
			send_games();
		}
	}elseif($_REQUEST['action']==='music'){
		check_session();
		if(isset($_REQUEST['loc']) && isset($_REQUEST['loop']) && isset($_REQUEST['mode'])){
			send_music($_REQUEST['loc'],$_REQUEST['loop'],$_REQUEST['mode']);
		}else{
			send_music();
		}
	}elseif($_REQUEST['action']==='coding'){
		check_session();
		send_coding($_REQUEST['lang'],$_REQUEST['method'],$_REQUEST['content']);
...
function send_admin($arg=''){
	...
	if($U['status']>=7){
		thr();
		echo '<tr><td>'.form_target('utility', 'setup').submit($I['initgosetup']).'</form></td></tr>';
	}
	thr();
...
function send_links(){
	global $I;
	print_start('links');
	echo "<center><h1>Links</h1>";
	echo "<hr /><p>Three scripts are available here:</p>
	<p>-<a href='index.php'>index.php</a>- stable modified version</p>
	<p>-<a href='dev.php'>dev.php</a>- used for testing new features</p>
	<p>-<a href='original.php'>original.php</a>- as it is on GitHub</p>
	<hr>
	<p>-<a href=''>MORE CHAT OPINION COMING SOON</a>- </p>
	<p>-<a href=''>MORE CHAT OPINION COMING SOON</a>- </p>
	<p>-<a href=''>MORE CHAT OPINION COMING SOON</a>- </p>
	<hr>
	<br>
...
function send_coding($lang='',$method='',$content=''){
	global $I;
	print_start('coding');
	echo "<center><h1>Encodings</h1>";
	echo "<table width=100%><tr><td><center>";
	echo form('coding').hidden('lang','b64').submit('Base64')."</form>
	</center></td><td><center>";
	echo form('coding').hidden('lang','b65k').submit('Base65536')."</form>";
//	echo "</center></td><td><center>";
//	echo form('coding').hidden('lang','enigma').submit('Enigma')."</form>
//	</center></td><td><center>";
//	echo form('coding').hidden('lang','encryption').submit('Encryption')."</form>";
	echo "</center></td></tr></table>";
	echo "<center><br><hr><br>";
	if($lang=='b64'){
//		echo "<h1>Base 64</h1><hr>";
//		echo "<p>Base64 encoder/decoder coming soon!</p>";
//		echo "<br><br><hr>";
		$encodedString = base64_encode('hello world'); // Output: aGVsbG8gd29ybGQ=
		$decodedString = base64_decode($encodedString); // Output: hello world 
		if($method=='encode'){
			$encodedString = base64_encode($content);
		...
		echo "<h1>Base 64</h1><hr>";
		echo "<p>Base64 encoder/decoder</p><hr>";
		echo "<table width=100%><tr><td><center><br>";
		echo form('coding').hidden('lang','b64').hidden('method','encode').submit('Encode Message')."<br>";
		echo "<textarea name='content' rows='4' cols='30' style='$U[style]' >".$decodedString."</textarea></form><br><hr><br>";
		echo form('coding').hidden('lang','b64').hidden('method','decode').submit('Decode Message');
		echo "<br><textarea name='content' rows='4' cols='30' style='$U[style]' >".$encodedString."</textarea></form><br></td>";
		echo "</center></tr></table><br><br><hr>";
	}
	if($lang=='b65k'){
		$encodedString = Base65536::encode('hello world'); // Output: 驨ꍬ啯𒁷ꍲᕤ
		$decodedString = Base65536::decode($encodedString); // Output: hello world 
		if($method=='encode'){
			$encodedString = Base65536::encode($content);
			$decodedString = Base65536::decode($encodedString);
		}elseif($method=='decode'){
			$decodedString = Base65536::decode($content);
			$encodedString = Base65536::encode($decodedString);
		}
		echo "<h1>Base 65536</h1><hr>";
		echo "<p>Base65536 encoder/decoder</p><hr>";
		echo "<table width=100%><tr><td><center><br>";
		echo form('coding').hidden('lang','b65k').hidden('method','encode').submit('Encode Message')."<br>";
		echo "<textarea name='content' rows='4' cols='30' style='$U[style]' >".$decodedString."</textarea></form><br><hr><br>";
		echo form('coding').hidden('lang','b65k').hidden('method','decode').submit('Decode Message');
		echo "<br><textarea name='content' rows='4' cols='30' style='$U[style]' >".$encodedString."</textarea></form><br></td>";
		echo "</center></tr></table><br><br><hr>";
	}
	if($lang=='enigma'){
		echo "<h1>Enigma</h1><hr>";
		echo "<p>Enigma machine emulator coming soon!</p>";
		echo "<br><br><hr>";
	}
	if($lang=='encryption'){
		echo "<h1>Encryption</h1><hr>";
		echo "<p>Encryption/decryption modules coming soon!</p>";
		echo "<br><br><hr>";
	}
	echo "<br><br><font size=2>Encodings</font size><br><br><hr></center>";
	print_end();
}
...
function send_game(){
	global $I;
	print_start('games');
	echo "<center><h1>Games</h1>";
	echo "<p>Several games will be added soon, starting with Card Sharks, a simple card game where you guess whether the next card's value will be higher or lower than the last card's value.</p>";
	echo "<p>If you have any ideas for games to add, feel free to send questions & comments to <strong>@Enforcer</strong> or <font color=#33CCFF><strong>@Curious</strong></font color></p>";
	echo "</center>";
	print_end();
}

function send_games($game='null',$match='null',$partner='',$choice='',$move=''){
	global $I;
	print_start('games');
	echo "<center><h1>Games</h1>";
//	echo "<p>Several games will be added soon, starting with Card Sharks, a simple card game where you guess whether the next card's value will be higher or lower than the last card's value.</p>";
	...
	if(($game=='null') && ($match=='null')){
		echo "<table width=100%><tr><td><center>";
		echo form('games').hidden('game','null').hidden('match','joinmatch').submit('Join Match')."</form>";
		echo "</center></td><td><center>";
		echo form('games').hidden('game','null').hidden('match','creatematch').submit('Create Match')."</form>";
		echo "</center></td><td><center>";
		echo "</center></td></tr></table>";
		echo "</tr></table>";
	}elseif(($game=='null') && ($match=='joinmatch')){
		echo "<table width=100%><tr><td><center>";
		echo "</h3>Not Yet Imlemented<h3>";
		echo form('games').hidden('game','null').hidden('match','null').submit('Back to Main Menu')."</form>";
		echo "</center></td></tr></table>";
	}elseif(($game=='null') && ($match=='creatematch')){
		echo "<table width=100%><tr><td><center>";
		echo form('games').hidden('game','battleship').hidden('match','creatematch').submit('Battleship')."</form>";
		echo "</center></td><td><center>";
		echo form('games').hidden('game','cards').hidden('match','creatematch').submit('Cards')."</form>";
		echo "</center></td><td><center>";
		echo form('games').hidden('game','checkers').hidden('match','creatematch').submit('Checkers')."</form>";
		echo "</center></td><td><center>";
		echo form('games').hidden('game','chess').hidden('match','creatematch').submit('Chess')."</form>";
		echo "</center></td><td><center>";
		echo form('games').hidden('game','tictactoe').hidden('match','creatematch').submit('Tic Tac Toe')."</form>";
		echo "</center></td></tr></table>";
	}elseif(($game=='battleship') && ($match=='creatematch')){
		echo "<table width=100%><tr><td><center>";
		echo "</h3>Not Yet Imlemented<h3>";
		echo form('games').hidden('game','null').hidden('match','creatematch').submit('Back to Create Match')."</form>";
		echo form('games').hidden('game','null').hidden('match','null').submit('Back to Main Menu')."</form>";
		echo "</center></td></tr></table>";
	}elseif(($game=='cards') && ($match=='creatematch')){
		echo "<table width=100%><tr><td><center>";
		echo "</h3>Not Yet Imlemented<h3>";
		echo form('games').hidden('game','null').hidden('match','creatematch').submit('Back to Create Match')."</form>";
		echo form('games').hidden('game','null').hidden('match','null').submit('Back to Main Menu')."</form>";
		echo "</center></td></tr></table>";
	}elseif(($game=='checkers') && ($match=='creatematch')){
		echo "<table width=100%><tr><td><center>";
		echo "</h3>Not Yet Imlemented<h3>";
		echo form('games').hidden('game','null').hidden('match','creatematch').submit('Back to Create Match')."</form>";
		echo form('games').hidden('game','null').hidden('match','null').submit('Back to Main Menu')."</form>";
		echo "</center></td></tr></table>";
	}elseif(($game=='chess') && ($match=='creatematch')){
		echo "<table width=100%><tr><td><center>";
		echo "</h3>Not Yet Imlemented<h3>";
		echo form('games').hidden('game','null').hidden('match','creatematch').submit('Back to Create Match')."</form>";
		echo form('games').hidden('game','null').hidden('match','null').submit('Back to Main Menu')."</form>";
		echo "</center></td></tr></table>";
	}elseif(($game=='tictactoe') && ($match=='creatematch')){
		echo "<table width=100%><tr><td><center>";
		new TicTacToe();
//		echo "</h3>Not Yet Imlemented<h3>";
//		echo form('games').hidden('game','null').hidden('match','creatematch').submit('Back to Create Match')."</form>";
//		echo form('games').hidden('game','null').hidden('match','null').submit('Back to Main Menu')."</form>";
		...
		echo "What were you doing? something just broke, lol";
	}
}
...
function send_music($loc='',$loop='off',$mode='links'){
	global $I;
	print_start('music');
	echo "<div class='musicbox' id='musicbox'>
			<table width=100%><thead>
			<center><h1>♫🎧♪ 🎼 Music Box 🎼 ♪🎧♫</h1>
			</thead><tr><td><center>";
	echo form('music').hidden('loc',$loc).hidden('loop',$loop).hidden('mode','links').submit('Links')."</form>
	</center></td><td><center>";
	if($mode==='listen'){
		echo"Mode : Listen";
	}else{
	...
	echo "</center></td><td><center>";
	echo form('music').hidden('loc',$loc).hidden('loop',$loop).hidden('mode','listen').submit('Listen')."</form>
	</center></td></tr><tr><td><center>";
	
	if($mode==='listen'){
		echo form('music').hidden('loc',$loc).hidden('loop','on').hidden('mode',$mode).submit($I['loop_on'])."</form>
		</center></td><td><center>";
		if($loop=='on'){
			echo"Loop : on";
		}else{
	...
	echo "</center></td><td><center>";
	echo form('music').hidden('loc',$loc).hidden('loop','off').hidden('mode',$mode).submit($I['loop_off'])."</form>
	</center></td></tr><tr><td><center>";
	}
	
	echo form('music').hidden('loc','').hidden('loop',$loop).hidden('mode',$mode).submit('Root')."</form>
	</center></td><td><center>";
	echo form('music').hidden('loc','explore').hidden('loop',$loop).hidden('mode',$mode).submit('Explore')."</form>
	</center></td><td><center>";
	echo form('music').hidden('loc','all').hidden('loop',$loop).hidden('mode',$mode).submit('All')."</form>
	</center></td></tr></table>";
	echo "<hr /><center>";
     foreach (array_filter(glob('music/*'), 'is_file') as $file){
		...
		}elseif($file_type=='ogg'){
			echo "type='audio/ogg'";
		}
		echo "Either your browser does not support the audio element, or you have audio tags disabled in NoScript.
		</audio></div>
		<hr />";
	}
	$path = 'https://mega.nz/folder/MywxFIxL#Vlu0qOuEyMls_znMzmm2LQ';
	if($loc=='explore'){
		...
		foreach ($folders as $folder) {
			...
			if (is_dir($path . $folder)) {
				...
				echo "<h2>Explorer</h2>";
				echo "<br>";
				echo form('music').hidden('loc',$folder).hidden('loop',$loop).hidden('mode',$mode).submit($folder)."</form><br><hr>";
			}
		}
	}elseif($loc=='all'){
		$folders = scandir($path);
		echo "<table border=1px width=90%><tr><td><center><h3>[ ".$path." ]</h3></center></td></tr>";
		foreach(glob($path.'/*.mp3') as $file) {
		$file_path = pathinfo($file);
			echo "<tr><td><center>";
			if($mode==='listen'){
				echo "			<div class='song' id='song'>
					<h3>♪ ".$file_path['filename']." ♪</h3>
					<audio  src='/music/".$file_path['basename']."' type='audio/mpeg' ";
				if($loop=='on'){
					echo"loop ";
				}
				echo "controls>
					Either your browser does not support the audio element, or you have media tags disabled.
					</audio></div>";
			}elseif($mode==='links'){
				echo "		<h3>♪ <a href='/music/".$file_path['basename']."' target='_blank' >".$file_path['filename']."</a> ♪</h3>";
			}
			echo "			<hr></center></td></tr>";
		}
		echo "<tr><td><center><h3>[ ".$path." ]</h3></center></td></tr>";
		echo "</table>";
		echo "<br><br>";
		foreach ($folders as $folder) {
			if ($folder === '.' or $folder === '..') continue;
			if (is_dir($path.$folder)) {
				echo "<table border=1px width=90%><tr><td><center><h3>[ ".$folder." ]</h3><hr>";
				foreach(glob($path.$folder.'/*.mp3') as $file) {
					$file_path = pathinfo($file);
					if($mode==='listen'){
						echo "<div class='song' id='song'>
							<h3>♪ ".$file_path['filename']." ♪</h3>
							<audio  src='/music/".$folder.'/'.$file_path['basename']."' type='audio/mpeg' ";
						if($loop=='on'){
							echo"loop ";
						}
						echo "controls>
						...
						echo "		<h3>♪ <a href='/music/".$folder."/".$file_path['basename']."' target='_blank' >".$file_path['filename']."</a> ♪</h3>";
					}
					echo "	<hr>";				}
				echo "<h3>[ ".$folder." ]</h3></center></td></tr>";
				echo "</table>";
			}
		}
	}else{
		echo "<table border=3px width=90%><tr><td><center><h3>[ ";
		if($loc!=''){
			echo $loc;
		}else{
			echo $path;
		...
		echo " ]</h3><hr>";
		foreach(glob($path.$loc.'/*.mp3') as $file) {
			$file_path = pathinfo($file);
			if($mode==='listen'){
			echo "	<div class='song' id='song'>
					<h3>♪ ".$file_path['filename']." ♪</h3>
				...
				echo $file_path['basename']."' type='audio/mpeg' ";
				if($loop=='on'){
					echo"loop ";
				}
				echo "controls>
					Either your browser does not support the audio element, or you have audio tags disabled in NoScript.
					</audio></div>";
			}elseif($mode==='links'){
				echo " 		<h3>♪ <a href='/music/".$loc;
					if($loc!=''){
						echo"/";
					}
				echo $file_path['basename']."' target='_blank' >".$file_path['filename']."</a> ♪</h3>";
			}
			echo "	<hr>";
		}
		echo "<h3>[ ";
...
function send_frameset(){
	...
	if((!isset($_REQUEST['sort']) && !$U['sortupdown']) || (isset($_REQUEST['sort']) && $_REQUEST['sort']==0)){
		echo "<frameset rows=\"$postheight,*,50\" border=\"3\" frameborder=\"3\" framespacing=\"3\">";
		echo "<frame name=\"post\" src=\"$_SERVER[SCRIPT_NAME]?action=post&session=$U[session]&lang=$language\">";
		echo "<frameset cols=\"60%,30%\" border=\"3\" frameborder=\"3\" framespacing=\"3\">";
		...
		echo "<frameset rows=\"60%,40%\" border=\"2\" frameborder=\"2\" framespacing=\"2\">";
		echo "<frame name=\"musicbox\" src=\"$_SERVER[SCRIPT_NAME]?action=music&session=$U[session]&lang=$language&sort=1\">";
		echo "<frame name=\"utility\" src=\"$_SERVER[SCRIPT_NAME]?action=help&session=$U[session]&lang=$language&sort=1\">";
		echo "</frameset>";
		echo "</frameset>";
		...
		echo "<frameset cols=\"60%,40%\" border=\"3\" frameborder=\"3\" framespacing=\"3\">";
		if(get_setting('enablegreeting')){
			echo "<frame name=\"view\" src=\"$_SERVER[SCRIPT_NAME]?action=greeting&session=$U[session]&lang=$language\">";
		}else{
			echo "<frame name=\"view\" src=\"$_SERVER[SCRIPT_NAME]?action=view&session=$U[session]&lang=$language$bottom\">";
		}
		echo "<frameset rows=\"15%,85%\" border=\"3\" frameborder=\"3\" framespacing=\"3\">";
		echo "<frame name=\"musicbox\" src=\"$_SERVER[SCRIPT_NAME]?action=music&session=$U[session]&lang=$language&sort=1\">";
		echo "<frame name=\"utility\" src=\"$_SERVER[SCRIPT_NAME]?action=help&session=$U[session]&lang=$language&sort=1\">";
		echo "</frameset>";
		echo "</frameset>";
...
function send_notes($type){
	...
	if($U['status']>=5 && ($personalnotes || $U['status']>6)){
		echo '<table><tr>';
		if($U['status']>6){
			echo '<td>'.form_target('utility', 'notes', 'admin').submit($I['admnotes']).'</form></td>';
		}
		echo '<td>'.form_target('utility', 'notes', 'staff').submit($I['staffnotes']).'</form></td>';
		if($personalnotes){
			echo '<td>'.form_target('utility', 'notes').submit($I['personalnotes']).'</form></td>';
		}
		echo '</tr></table>';
	}
	if($type===1){
...
function send_choose_messages(){
	...
	echo form_target('utility', 'admin').submit($I['adminbtn']).'</form><br><br>';
	echo form('admin', 'clean');
	echo hidden('what', 'selected').submit($I['delselmes'], 'class="delbutton"');
	print_messages($U['status']);
	echo '<br>'.submit($I['delselmes'], 'class="delbutton"')."</form><br><br>";
	echo form_target('utility', 'admin').submit($I['adminbtn']).'</form>';
	print_end();
}
...
function send_post($rejected=''){
	...
	if($U['status']>=2){
		echo '<option ';
		if($_REQUEST['sendto']==='s rg'){
			echo 'selected ';
		}
		echo "value=\"s rg\">-$I[toreg]-</option>";
	}
	if($U['status']>=3){
		echo '<option ';
		if($_REQUEST['sendto']==='s ?'){
	...
	if($U['status']>=6){
		echo '<option ';
		if($_REQUEST['sendto']==='s sup'){
			echo 'selected ';
		}
		echo "value=\"s sup;\">-$I[tosupmod]-</option>";
	}
	if($U['status']>=7){
		echo '<option ';
		if($_REQUEST['sendto']==='s &'){
	...
	if($U['status']>=9){
		...
		echo "value=\"s ++;\">-$I[tobots10]-</option>";
	}
...
function send_profile($arg=''){
	...
	echo "<tr><td><table id=\"colour\"><tr><th>$I[fontcolour] (<a href=\"$_SERVER[SCRIPT_NAME]?action=colours&amp;session=$U[session]&amp;lang=$language\" target=\"utility\">$I[viewexample]</a>)</th><td>";
	echo "<input type=\"color\" value=\"#$matches[1]\" name=\"colour\"></td></tr></table></td></tr>";
	thr();
	echo "<tr><td><table id=\"bgcolour\"><tr><th>$I[bgcolour] (<a href=\"$_SERVER[SCRIPT_NAME]?action=colours&amp;session=$U[session]&amp;lang=$language\" target=\"utility\">$I[viewexample]</a>)</th><td>";
...
function send_controls(){
	...
	echo '<td>'.form_target('utility', 'profile').submit($I['chgprofile']).'</form></td>';
		echo '<td>'.form_target('utility', 'games').submit('Games').'</form></td>';
	echo '<td>'.form_target('musicbox', 'music').hidden('loc','').submit('Music').'</form></td>';
//	if($U['status']>=3){
		echo '<td>'.form_target('utility', 'coding').hidden('lang','').submit('Code').'</form></td>';
	if($U['status']>=3){
		if($personalnotes){
			echo '<td>'.form_target('utility', 'notes').submit($I['notes']).'</form></td>';
		}
	}
//	}
	if($U['status']>=5){
    ...
    echo '<td><form action="/emojis.html" method="post" target="view"><input type="hidden" name="lang" value="en"><input type="hidden" name="nc" value="512842"><input type="hidden" name="action" value="view"><input type="hidden" name="session" value="fd3a73795ff5395e4893de101013da72"><input type="submit" value="Emojis" ></form></td>';
	echo '<td><form action="/credit.html" method="post" target="view"><input type="hidden" name="lang" value="en"><input type="hidden" name="nc" value="512842"><input type="hidden" name="action" value="view"><input type="hidden" name="session" value="fd3a73795ff5395e4893de101013da72"><input type="submit" value="&copy;" ></form></td>';
	echo '<td><form action="http://cboxkuuxrtulkkxhod2pxo3la25tztcp4cdjmc75wc5airqqliq2srad.onion/music.php" method="post" target="_parent"><input type="hidden" name="lang" value="en"><input type="hidden" name="nc" value="512842"><input type="hidden" name="action" value="login"><input type="hidden" name="frameset" value="'.$U['frameset'].'"><input type="hidden" name="session" value="'.$U['session'].'"><input type="submit" id="onionbutton" value="Onion Site" ></form></td>';
	echo '<td><form action="/index.php" method="post" target="_parent"><input type="hidden" name="lang" value="en"><input type="hidden" name="nc" value="512842"><input type="hidden" name="action" value="login"><input type="hidden" name="frameset" value="'.$U['frameset'].'"><input type="hidden" name="session" value="'.$U['session'].'"><input type="submit" id="onionbutton" value="Normal Version" ></form></td>';
...
function validate_input(){
	...
	}elseif($_REQUEST['sendto']==='s rg' && $U['status']>=2){
		$poststatus=2;
		$displaysend=sprintf(get_setting('msgsendreg'), style_this(htmlspecialchars($U['nickname']), $U['style']));
	...
	}elseif($_REQUEST['sendto']==='s \+' && $U['status']>=4){
		$poststatus=4;
		$displaysend=sprintf(get_setting('msgsendbots'), style_this(htmlspecialchars($U['nickname']), $U['style']));
	...
	}elseif($_REQUEST['sendto']==='s sup' && $U['status']>=6){
		$poststatus=6;
		$displaysend=sprintf(get_setting('msgsendsuper'), style_this(htmlspecialchars($U['nickname']), $U['style']));
	}elseif($_REQUEST['sendto']==='s &' && $U['status']>=7){
		$poststatus=7;
		$displaysend=sprintf(get_setting('msgsendadm'), style_this(htmlspecialchars($U['nickname']), $U['style']));
	}elseif($_REQUEST['sendto']==='s \+\+' && $U['status']>=10){
		$poststatus=10;
		$displaysend=sprintf(get_setting('msgsendoverseer'), style_this(htmlspecialchars($U['nickname']), $U['style']));
	...
	if(isset($_FILES['file']) && get_setting('enfileupload')){
		if($_FILES['file']['error']===UPLOAD_ERR_OK && $_FILES['file']['size']<=(1024*get_setting('maxuploadsize'))){
			...
			$message=sprintf(get_setting('msgattache'), "<a class=\"attachement\" href=\"$_SERVER[SCRIPT_NAME]?action=download&amp;id=$hash\" target=\"utility\">$name</a>", $message);
		}
	}
...
function apply_linkfilter($message){
	$filters=get_linkfilters();
	foreach($filters as $filter){
		$message=preg_replace_callback("/<a href=\"([^\"]+)\" target=\"utility\">(.*?(?=<\/a>))<\/a>/iu",
			function ($matched) use(&$filter){
				return "<a href=\"$matched[1]\" target=\"utility\">".preg_replace("/$filter[match]/iu", $filter['replace'], $matched[2]).'</a>';
			}
		, $message);
	}
	$redirect=get_setting('redirect');
	if(get_setting('imgembed')){
		$message=preg_replace_callback('/\[img\]\s?<a href="([^"]+)" target="utility">(.*?(?=<\/a>))<\/a>/iu',
			function ($matched){
				return str_ireplace('[/img]', '', "<br><a href=\"$matched[1]\" target=\"utility\"><img src=\"$matched[1]\"></a><br>");
			}
		, $message);
	}
	if(empty($redirect)){
	...
	if(get_setting('forceredirect')){
		$message=preg_replace_callback('/<a href="([^"]+)" target="utility">(.*?(?=<\/a>))<\/a>/u',
			function ($matched) use($redirect){
				return "<a href=\"$redirect".rawurlencode($matched[1])."\" target=\"utility\">$matched[2]</a>";
			}
		, $message);
	}elseif(preg_match_all('/<a href="([^"]+)" target="utility">(.*?(?=<\/a>))<\/a>/u', $message, $matches)){
		foreach($matches[1] as $match){
			if(!preg_match('~^http(s)?://~u', $match)){
				$message=preg_replace_callback('/<a href="('.preg_quote($match, '/').')\" target=\"utility\">(.*?(?=<\/a>))<\/a>/u',
					function ($matched) use($redirect){
						return "<a href=\"$redirect".rawurlencode($matched[1])."\" target=\"utility\">$matched[2]</a>";
					}
				, $message);
	...
	$message=preg_replace_callback('/<<([^<>]+)>>/u',
		function ($matches){
			if(strpos($matches[1], '://')===false){
				return "<a href=\"http://$matches[1]\" target=\"utility\">$matches[1]</a>";
			}else{
				return "<a href=\"$matches[1]\" target=\"utility\">$matches[1]</a>";
			}
		}
	, $message);
	return $message;
...
function init_chat(){
	...
	if(check_init()){
		...
		$settings=[
			...
			['msgsendreg', $I['sendregmsg']],
			['msgsendmem', $I['sendmemmsg']],
			['msgsendbot', $I['sendbotmsg']],
			['msgsendmod', $I['sendmodmsg']],
			['msgsendsup', $I['sendsupmsg']],
			['msgsendadm', $I['sendadmmsg']],
			['msgsendprv', $I['sendprvmsg']],
			['msgsendovr', $I['sendovrmsg']],
			['msgenter', $I['entermsg']],
			['msgexit', $I['exitmsg']],
...
function load_config(){
	...
	define('ENCRYPTKEY', 'r62pzYn0MCvrGVrk6XWs7CJ1A9gXtPB2XMR1KBllMtqYjtNFnFWGX4Mip5fHglsU3JUFQlqNEWAjvzCwkAQGDnkPBQ4UfNkSUuciyjZrty0fJ8qlzXzAy27evO15Zpzb'); // Encryption key for messages
	...
	define('PREFIX', 'music_'); // Prefix - Set this to a unique value for every chat, if you have more than 1 chats on the same database or domain - use only alpha-numeric values (A-Z, a-z, 0-9, or _) other symbols might break the queries
